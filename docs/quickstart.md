...
Copy `src/mongreldb.v` into your project's module path (e.g.
...
fn main() {
	...
	tid := db.create_table('orders', [
		...
		mongreldb.Column{4, 'status', 'varchar', false, false, ['pending', 'shipped', 'delivered', 'cancelled'], ''},
		// Enum column with a default applied when the cell is omitted.
		mongreldb.Column{5, 'currency', 'varchar', false, false, ['USD', 'EUR', 'GBP'], 'USD'},
	]) or { panic(err) }
	...
	q = q.where_('range', {'column': mongreldb.int_value(3), 'min': mongreldb.float_value(100.0)})
	q = q.projection([1, 2])
	q = q.limit_(100)
